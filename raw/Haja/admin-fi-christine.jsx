...
// ════════════════════════════════════════════════════════════════
// LOI · Admin-Fi CHRISTINE — Workspace D82
// Exécutant Admin-Fi #3 · CAISSE & CAHIER D'ARRIVÉE
// Archétype identique aux workspaces Sarobidy/Belaza · contenu métier swappé.
// ════════════════════════════════════════════════════════════════
...
const TYPE_META = {
  decaisser:   { label: "DDV caisse",        color: "var(--accent)",  shortName: "Décaisser espèces" },
  ...
  anomalie:    { label: "Écart caisse",      color: "var(--err)",     shortName: "Signaler" },
};

function actionLabelFor(item) {
  if (item.type === "decaisser")   return "Décaisser espèces";
  if (item.type === "enregistrer") return "Enregistrer";
  if (item.type === "paye")        return "Marquer payé";
  if (item.type === "anomalie")    return "Signaler";
  return "Traiter";
}

// File de travail — 7 lignes · 2 overdue · somme = 5.81 M MGA
const QUEUE_SEED = [
  { id: "c-001", type: "decaisser",   ref: "DDV-C-2401", demandeur: "Édienne",   dept: "Transit Toamasina", montant: "0.80 M MGA",  raw: 0.80,  etape: "decaissement", ageJours: 3, slaJours: 2 },
  { id: "c-003", type: "anomalie",    ref: "ECART-T-08", demandeur: "Christine", dept: "Caisse Toamasina",  montant: "0.008 M MGA", raw: 0.008, etape: "matrice",      ageJours: 3, slaJours: 2 },
  { id: "c-007", type: "decaisser",   ref: "DDV-C-2422", demandeur: "Haja",      dept: "Direction",         montant: "2.40 M MGA",  raw: 2.40,  etape: "decaissement", ageJours: 1, slaJours: 2 },
  { id: "c-002", type: "enregistrer", ref: "DDV-C-2410", demandeur: "Rico",      dept: "Achat",             montant: "1.20 M MGA",  raw: 1.20,  etape: "demande",      ageJours: 2, slaJours: 3 },
  { id: "c-004", type: "decaisser",   ref: "DDV-C-2415", demandeur: "Belaza",    dept: "Admin-Fi",          montant: "0.50 M MGA",  raw: 0.50,  etape: "decaissement", ageJours: 1, slaJours: 2 },
  { id: "c-006", type: "enregistrer", ref: "DDV-C-2420", demandeur: "Lynda",     dept: "RH",                montant: "0.60 M MGA",  raw: 0.60,  etape: "demande",      ageJours: 1, slaJours: 3 },
  { id: "c-005", type: "paye",        ref: "DDV-C-2398", demandeur: "Ketsiah",   dept: "RH",                montant: "0.30 M MGA",  raw: 0.30,  etape: "paye",         ageJours: 0, slaJours: 1 },
];
...
const HERO_KPIS = (() => {
  ...
  return [
    { id: "h1", label: "DDV ouvertes",   value: open,    unit: "objets",          tone: "neutral", icon: "clipboard" },
...
const COACHING = [
  {
    id: "sig-1", icon: "alert",
    cause: "Écart caisse Toamasina 8K MGA · 2e incident du mois",
    levier: "Double-comptage caisse en fin de journée + durcir le cahier d'arrivée",
    gain: "0 écart caisse · sécurise la confiance terrain",
    source: "Terrain Toamasina",
  },
  {
    id: "sig-2", icon: "clock",
    cause: "Quelques DDV caisse payées >24h",
    levier: "Traiter les appro caisse dès le matin",
    gain: "Tenir le délai paid<24h · sécurise Objectif 1 Cash",
    source: "Haja",
  },
  {
    id: "sig-3", icon: "bolt",
    cause: "Deadlines achats consommables serrées",
    levier: "Anticiper la commande à J-3",
    gain: "0 rupture office · fluidité quotidienne",
    source: "Christine",
  },
];

const CARNET = [
  { id: "c-1", date: "2026-05-22", type: "decision", text: "Mise en place double-comptage caisse Toamasina J+1",            source: "Haja" },
  { id: "c-2", date: "2026-05-19", type: "escalade", text: "Écart caisse Toamasina 8K MGA · escalade Haja pour arbitrage", source: "Christine" },
  { id: "c-3", date: "2026-05-14", type: "decision", text: "Routine appro caisse · traitement matin obligatoire",          source: "Belaza" },
];
...
const SO_KPI = {
  label: "DDV caisse payées <24h",
  value: 92, target: 95, unit: "%",
  hint: "% sous SLA · 12 semaines glissantes",
  series: [82, 84, 86, 85, 88, 87, 89, 90, 88, 91, 90, 92],
};

// Conformité — scope Christine (caisse + cahier)
const COMPLIANCE = [
  { id: "cahier",    label: "Cahier d'arrivée à jour", current: "J-0", total: "J-0", unit: "",      status: "ok",   note: "Aucun retard d'enregistrement" },
  { id: "anomalies", label: "Anomalies caisse ce mois", current: 1,     total: 0,     unit: "incident", status: "warn", note: "1 écart Toamasina · cible 0" },
];

// Scores perso Christine
const PERSO_KPIS = [
  { id: "C-K01", label: "DDV caisse payées <24h",     value: 92, target: 95, status: "warn", series: [82, 84, 86, 85, 88, 87, 89, 90, 88, 91, 90, 92], unit: "%" },
  { id: "C-K02", label: "Cahier d'arrivée à jour J-0", value: "OK", target: "OK", status: "ok", series: null, unit: "" },
  { id: "C-K03", label: "Anomalies caisse / mois",     value: 1,  target: 0,  status: "warn", series: [0, 0, 1, 0, 0, 0, 1, 0, 0, 1, 0, 1], unit: "" },
];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 6 }}>
      ...
      <div style={{ fontSize: 22, fontWeight: 700, color: "var(--fg-1)", letterSpacing: "-0.015em", display: "flex", alignItems: "center", gap: 10 }}>
        WORKSPACE CHRISTINE <span style={{ color: "var(--accent)" }}>·</span> CAISSE & CAHIER D'ARRIVÉE
      </div>
      <div style={{ fontSize: 14, color: "var(--fg-3)" }}>Caisse · cahier d'arrivée · deadline · achat · drill-out Haja</div>
    </div>
  );
}
    ...
    <div style={{
      ...
      <div style={{ position: "relative", flexShrink: 0 }}>
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
          ...
          <circle cx={size/2} cy={size/2} r={r} fill="none" stroke="url(#soGradChr)" strokeWidth={stroke} strokeLinecap="round" strokeDasharray={`${c * ratio} ${c}`} transform={`rotate(-90 ${size/2} ${size/2})`} style={{ filter: "drop-shadow(0 0 8px color-mix(in srgb, var(--accent) 70%, transparent))" }}/>
          <defs><linearGradient id="soGradChr" x1="0" x2="1" y1="0" y2="1"><stop offset="0%" stopColor="var(--accent)"/><stop offset="100%" stopColor="var(--pl-teal-light)"/></linearGradient></defs>
        </svg>
...
// ───────────────────────────────────────────────────────────
// Caisse & contrôle block — 2 cards (scope Christine)
// ───────────────────────────────────────────────────────────
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
        ...
        {isCount && <span style={{ fontFamily: "var(--font-mono)", fontSize: 14, color: "var(--fg-3)" }}>/ cible {k.total} {k.unit}</span>}
  ...
  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 18, boxShadow: "var(--shadow-1)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
        ...
        <h3 style={{ margin: 0, fontSize: 17, fontWeight: 700, color: "var(--fg-1)", letterSpacing: "0.01em", textTransform: "uppercase" }}>Caisse & contrôle</h3>
        <span style={{ fontSize: 13, color: "var(--fg-3)" }}>(hors SO · scope Christine)</span>
      </div>
...
// ───────────────────────────────────────────────────────────
// Action Bar — 4 boutons Christine
// ───────────────────────────────────────────────────────────
...
function ActionBar({ onEnregistrer, onDecaisser, onMarquer, onSignaler }) {
  return (
    <div style={{
      ...
      <ActionButton label="Décaisser espèces"       variant="accent"  onClick={onDecaisser}/>
      <ActionButton label="Marquer payé + classer"  variant="ok"      onClick={onMarquer}/>
      <ActionButton label="Signaler anomalie"       variant="danger"  onClick={onSignaler}/>
    </div>
  );
}
  ...
  return (
    <header style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <span style={{ fontSize: 18, fontWeight: 700, color: "var(--pl-teal-light)" }}>Christine</span>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div style={{ width: 36, height: 36, borderRadius: "50%", background: "var(--pl-teal)", display: "grid", placeItems: "center", color: "var(--pl-navy-deep)", fontWeight: 700, fontSize: 15 }}>C</div>
      </div>
    </header>
  );
}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 18, boxShadow: "var(--shadow-1)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
          <div style={{ width: 4, height: 18, background: "var(--accent)", borderRadius: 2 }}/>
          <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: "var(--fg-1)", textTransform: "uppercase", letterSpacing: "0.01em" }}>Tendance · DDV caisse payées &lt;24h</h3>
        </div>
        <Sparkline series={PERSO_KPIS[0].series} color="var(--warn)" target={95} width={1000} height={120}/>
        ...
        <div style={{ marginTop: 16, padding: 12, background: "var(--bg-elev-1)", borderRadius: 8, fontSize: 13, color: "var(--fg-2)" }}>
          <strong style={{ color: "var(--fg-1)" }}>Historique anomalies caisse</strong> — 4 incidents sur 12 semaines · 2 résolus en J+1 · 2 en cours d'analyse · routine double-comptage active depuis S-22.
        </div>
      </div>
    </div>
  );
...
function ProfilTab() {
  const fonctions = [ "Caisse · décaissement espèces", "Cahier d'arrivée · enregistrement", "Anomalies caisse · signalement", "Achat consommables office" ];
  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 26, display: "flex", gap: 24, alignItems: "flex-start", flexWrap: "wrap" }}>
      <div style={{ width: 96, height: 96, borderRadius: "50%", background: "linear-gradient(135deg, var(--accent) 0%, var(--pl-navy) 100%)", display: "grid", placeItems: "center", color: "white", fontSize: 40, fontWeight: 600 }}>C</div>
      <div style={{ flex: 1, minWidth: 280, display: "flex", flexDirection: "column", gap: 12 }}>
        <div>
          <div style={{ fontSize: 24, fontWeight: 700, color: "var(--fg-1)", letterSpacing: "-0.015em" }}>Christine</div>
          <div style={{ fontSize: 14, color: "var(--fg-3)" }}>Admin-Fi · Exécutant #3 · drill-out Haja</div>
        </div>
...
function App() {
  ...
  const [openDecaisser, setOpenDecaisser] = useState(false);
  const [openMarquer, setOpenMarquer] = useState(false);
  const [openSignaler, setOpenSignaler] = useState(false);
  ...
  const onEnregistrerSubmit = (data) => {
    ...
    pushToast(`✓ ${data.numeroSerie || "Entrée"} enregistrée`, `Objet : ${data.objet || "À CONFIRMER"} · cahier d'arrivée à jour`);
    advanceByType("enregistrer");
  };
  const onDecaisserSubmit = (data) => {
    setOpenDecaisser(false);
    pushToast(`✓ Décaissement espèces ${data.numeroSerie || "DDV"}`, `Mode : ${data.modePaiement || "espèces"} · trace caisse mise à jour`);
    advanceByType("decaisser");
  };
  ...
  const onSignalerSubmit = (data) => {
    setOpenSignaler(false);
    pushToast(`✓ Anomalie signalée`, `Type : ${data.type || "écart caisse"} · transmise Haja`);
    advanceByType("anomalie");
  };

  const handleQueueAction = (item) => {
    if (item.type === "enregistrer") setOpenEnregistrer(true);
    else if (item.type === "decaisser") setOpenDecaisser(true);
    else if (item.type === "paye") setOpenMarquer(true);
    else if (item.type === "anomalie") setOpenSignaler(true);
  };

  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--fg-1)", display: "flex", flexDirection: "column" }} data-screen-label="Admin-Fi Christine Workspace">
            ...
            {tab === "vue" && (
              <>
                ...
                <ActionBar
                  ...
                  onSignaler={() => setOpenSignaler(true)}
                />
                <ComplianceBlock/>
      ...
      <footer style={{
        ...
        <span>LOI · Admin-Fi Christine · exécutant #3</span>
      ...
      <DeclarationModal open={openSignaler}     onClose={() => setOpenSignaler(false)}     onSubmit={onSignalerSubmit}/>
    </div>
  );
}
