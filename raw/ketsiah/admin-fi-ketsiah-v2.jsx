...
// ★ HERO RENTABILITÉ & BUDGET — top focus · ce qui est PROPRE à Ketsiah
// (1 idée par carte · grand chiffre · gauge actuel vs cible)
const HERO_RENT = [
  {
    id: "marge", label: "Marge consolidée",
    value: 25.4, valueDisplay: "25,4", unit: "%",
    target: 28, targetDisplay: "28 %",
    ecart: "−2,6 pts", ecartTone: "warn",
    // Gauge math : 0 → 35 % de marge ; cible à 28
    gaugeMin: 0, gaugeMax: 35,
    status: "warn", icon: "pillar-cash",
    series: [22, 22, 23, 23, 24, 24, 25, 25, 25, 25, 25.2, 25.4],
    caption: "Trajectoire 12 semaines · marge en hausse lente · cible Q3 non atteinte",
  },
  {
    id: "budget", label: "Budget vs estimé",
    value: 3.8, valueDisplay: "+3,8", unit: "%",
    target: 2, targetDisplay: "±2 %",
    ecart: "+1,8 pt", ecartTone: "warn",
    // Gauge : -2 → +6 (zéro = parfait)
    gaugeMin: 0, gaugeMax: 6,
    invertGauge: true, // higher = worse
    status: "warn", icon: "chart",
    series: [1.2, 1.5, 1.8, 2.0, 2.4, 2.7, 2.9, 3.2, 3.4, 3.6, 3.7, 3.8],
    caption: "Dérive depuis S-7 · OPEX support · sortir de la zone tolérance ±2 %",
  },
];

// Rentabilité — détail (rangée secondaire) : marge par dépt + turnover
const MARGE_PAR_DEPT = {
  baked: true,
  rows: [
    { dept: "Transit",   v: 31, t: 29 },
    { dept: "Achat",     v: 28, t: 27 },
    { dept: "Caisse",    v: 24, t: 26 },
    { dept: "Direction", v: 22, t: 28 },
    { dept: "Admin-Fi",  v: 21, t: 26 },
  ],
};
const TURNOVER_B17 = {
  value: 8.0, valueDisplay: "8,0", unit: "%",
  target: 9, targetDisplay: "≤9 %",
  // turnover lower is better. gauge max set at 15 for context.
  gaugeMin: 0, gaugeMax: 15,
  status: "ok",
  series: [12, 11, 11, 10, 10, 9, 9, 9, 8, 8, 8, 8],
  caption: "Agrégat anonymisé · jamais nominatif (B17) · sous cible depuis 5 semaines",
  ecart: "−1 pt sous cible", ecartTone: "ok",
};
  ...
  return (
    <header style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        ...
        <span style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "color-mix(in srgb, var(--pl-cream) 80%, transparent)", letterSpacing: "0.08em", fontWeight: 700, textTransform: "uppercase",
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
          ...
          {focus && <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.14em", padding: "3px 7px", borderRadius: 4, background: "var(--accent)", color: "var(--accent-fg)", textTransform: "uppercase", fontFamily: "var(--font-mono)" }} title={`OWNE · ${p.owne}`}>OWNE</span>}
          {p.watch && !focus && <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.14em", padding: "3px 7px", borderRadius: 4, background: "var(--warn-soft)", color: "var(--warn)", textTransform: "uppercase", fontFamily: "var(--font-mono)" }} title={`WATCH · ${p.watch}`}>WATCH</span>}
        </div>
...
function PillarBar() {
  return (
    <div style={{ background: "var(--bg-elev-1)", borderBottom: "1px solid var(--border)", padding: "32px 56px" }}>
...
function CockpitHeader() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 4 }}>
      ...
      <div style={{ fontSize: 56, fontWeight: 700, color: "var(--fg-1)", letterSpacing: "-0.035em", lineHeight: 1, display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
        COCKPIT KETSIAH <span style={{ color: "var(--accent)", fontWeight: 400, opacity: 0.6 }}>·</span> <span style={{ color: "var(--fg-2)" }}>CFO</span>
      </div>
      <div style={{ fontSize: 18, color: "var(--fg-2)", lineHeight: 1.55, maxWidth: 920 }}>
...
// Hero KPIs (3 cards)
// ───────────────────────────────────────────────────────────
// ★ HERO RENTABILITÉ & BUDGET — 2 grandes cartes · 1 idée · grand chiffre + jauge
function Gauge({ value, min, max, target, invert }) {
  const pct      = Math.max(0, Math.min(100, ((value - min) / (max - min)) * 100));
  const targetPct = Math.max(0, Math.min(100, ((target - min) / (max - min)) * 100));
  const ok = invert ? value <= target : value >= target;
  return (
    <div style={{ position: "relative", height: 18, borderRadius: 999, background: "var(--bg-elev-1)", overflow: "visible", border: "1px solid var(--border-soft)" }}>
      <div style={{
        position: "absolute", inset: "1px", borderRadius: 999, width: `calc(${pct}% - 2px)`,
        background: ok ? "var(--ok)" : "var(--warn)",
        transition: "width 500ms",
      }}/>
      {/* Cible marker */}
      <div style={{
        position: "absolute", left: `${targetPct}%`, top: -7, bottom: -7, width: 2, marginLeft: -1,
        background: "var(--fg-1)", borderRadius: 1,
      }}/>
      <div style={{
        position: "absolute", left: `${targetPct}%`, top: -24, transform: "translateX(-50%)",
        fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--fg-2)", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", whiteSpace: "nowrap",
      }}>cible</div>
    </div>
  );
}

function GaugeBigCard({ k }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 5, background: tone.color }}/>
      {/* Header */}
      <div style={{ paddingLeft: 12, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14, minWidth: 0 }}>
          <div style={{ width: 42, height: 42, borderRadius: 11, background: tone.bg, color: tone.color, display: "grid", placeItems: "center", flexShrink: 0 }}>
            <AfIcon name={k.icon} size={22}/>
          </div>
          <span style={{ fontSize: 14, fontFamily: "var(--font-mono)", color: "var(--fg-2)", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: 700 }}>{k.label}</span>
        </div>
        <span style={{
          padding: "6px 14px", borderRadius: 999, fontSize: 13, fontFamily: "var(--font-mono)", fontWeight: 700,
          background: k.ecartTone === "ok" ? "var(--ok-soft)" : "var(--warn-soft)",
          color: k.ecartTone === "ok" ? "var(--ok)" : "var(--warn)",
          border: `1px solid color-mix(in srgb, ${k.ecartTone === "ok" ? "var(--ok)" : "var(--warn)"} 35%, transparent)`,
          whiteSpace: "nowrap", flexShrink: 0,
        }}>écart {k.ecart}</span>
      </div>

      {/* Big number */}
      <div style={{ paddingLeft: 12, display: "flex", alignItems: "baseline", gap: 14, flexWrap: "wrap", rowGap: 10 }}>
        <span style={{ fontFamily: "var(--font-mono)", fontSize: 92, fontWeight: 600, color: tone.color, letterSpacing: "-0.045em", lineHeight: 0.9, fontVariantNumeric: "tabular-nums" }}>{k.valueDisplay}</span>
        <span style={{ fontFamily: "var(--font-mono)", fontSize: 28, color: "var(--fg-3)", fontWeight: 500 }}>{k.unit}</span>
        <span style={{ display: "inline-flex", alignItems: "baseline", gap: 7, padding: "7px 13px", borderRadius: 8, background: "var(--bg-elev-1)", border: "1px solid var(--border-soft)", whiteSpace: "nowrap" }}>
          <span style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--fg-3)", textTransform: "uppercase", letterSpacing: "0.12em", fontWeight: 700 }}>cible</span>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 22, fontWeight: 700, color: "var(--fg-1)", letterSpacing: "-0.02em" }}>{k.targetDisplay}</span>
        </span>
      </div>

      {/* Gauge */}
      <div style={{ paddingLeft: 12, paddingTop: 8 }}>
        <Gauge value={k.value} min={k.gaugeMin} max={k.gaugeMax} target={k.target} invert={k.invertGauge}/>
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--fg-3)" }}>
          <span>{k.gaugeMin}{k.unit}</span><span>{k.gaugeMax}{k.unit}</span>
        </div>
      </div>

      {/* Caption */}
      <div style={{ paddingLeft: 12, fontSize: 14, color: "var(--fg-3)", lineHeight: 1.6, marginTop: "auto" }}>{k.caption}</div>
    </div>
  );
}

function HeroRentBlock() {
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24, flexWrap: "wrap" }}>
        <div style={{ width: 5, height: 32, background: "var(--accent)", borderRadius: 2 }}/>
        <h2 style={{ margin: 0, fontSize: 30, fontWeight: 700, color: "var(--fg-1)", letterSpacing: "-0.02em", textTransform: "uppercase" }}>★ Rentabilité &amp; budget</h2>
        <span style={{ fontSize: 15, color: "var(--fg-3)" }}>(propriété CFO · 1 domicile)</span>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(420px, 1fr))", gap: 24 }}>
        {HERO_RENT.map((k) => <GaugeBigCard key={k.id} k={k}/>)}
      </div>
    </div>
  );
}
...
function SOBand() {
  ...
  return (
    <div style={{
      ...
      borderRadius: 16, padding: "26px 32px", color: "var(--pl-cream)",
      border: "1px solid color-mix(in srgb, var(--accent) 22%, var(--pl-navy))",
      display: "grid", gridTemplateColumns: "minmax(200px, 0.8fr) minmax(240px, 1fr) minmax(240px, 1fr)", gap: 32, alignItems: "center",
    }}>
      {/* Identity */}
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          <span style={{ fontSize: 12, fontFamily: "var(--font-mono)", color: "color-mix(in srgb, var(--pl-cream) 65%, transparent)", textTransform: "uppercase", letterSpacing: "0.14em", fontWeight: 700 }}>{SO_DATA.q} · objectif</span>
          <span style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <span style={{ padding: "6px 13px", borderRadius: 7, background: "var(--accent)", color: "var(--accent-fg)", fontFamily: "var(--font-mono)", fontSize: 17, fontWeight: 700, letterSpacing: "-0.01em" }}>{SO_DATA.code}</span>
          <span style={{ fontSize: 13, fontFamily: "var(--font-mono)", color: "var(--pl-teal-light)", textTransform: "uppercase", letterSpacing: "0.12em", fontWeight: 700 }}>Conversion en cash</span>
        </div>
        <div style={{ fontSize: 18, fontWeight: 600, color: "var(--pl-cream)", letterSpacing: "-0.01em", lineHeight: 1.3, marginTop: 4 }}>{SO_DATA.title}</div>
        <div style={{ fontSize: 13, color: "color-mix(in srgb, var(--pl-cream) 60%, transparent)", lineHeight: 1.5 }}>
          Trajectoire · détail 6 banques chez Haja · co-mention SO·2
        </div>
      </div>
      ...
      <SOTrajectory
        ...
        series={SO_DSO.series}
      />
      <SOTrajectory
        label={SO_BGFI.label} value={SO_BGFI.value} unit={SO_BGFI.unit} target={`≥${SO_BGFI.target} %`}
        ecart={`−${bgfiEcart} pts`} ecartTone="warn"
        series={SO_BGFI.series}
      />
    </div>
  );
}

function SOTrajectory({ label, value, unit, target, ecart, ecartTone, series }) {
  const tColor = ecartTone === "err" ? "#FFB4A3" : ecartTone === "warn" ? "#FBD38D" : "var(--pl-teal-light)";
  return (
    <div style={{
      display: "flex", flexDirection: "column", gap: 10,
      borderLeft: "1px solid color-mix(in srgb, var(--pl-cream) 14%, transparent)", paddingLeft: 24,
    }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
        <span style={{ fontSize: 12, fontFamily: "var(--font-mono)", color: "color-mix(in srgb, var(--pl-cream) 75%, transparent)", letterSpacing: "0.12em", textTransform: "uppercase", fontWeight: 700 }}>{label}</span>
        <span style={{
          padding: "3px 10px", borderRadius: 999, fontSize: 11, fontFamily: "var(--font-mono)", fontWeight: 700,
          background: "color-mix(in srgb, var(--err) 22%, transparent)", color: tColor,
          border: `1px solid color-mix(in srgb, var(--err) 45%, transparent)`,
        }}>écart {ecart}</span>
      </div>
      <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
        <span style={{ fontFamily: "var(--font-mono)", fontSize: 52, fontWeight: 600, color: "var(--accent)", lineHeight: 0.95, letterSpacing: "-0.035em" }}>
          {value}<span style={{ fontSize: 20, color: "color-mix(in srgb, var(--pl-cream) 70%, transparent)", marginLeft: 4 }}>{unit}</span>
        </span>
        <span style={{ fontSize: 12, color: "color-mix(in srgb, var(--pl-cream) 60%, transparent)", fontFamily: "var(--font-mono)", marginLeft: "auto" }}>cible {target}</span>
      </div>
      <Sparkline series={series} color="var(--pl-teal-light)" target={null} width={300} height={32}/>
    </div>
  );
}
...
// Rentabilité — détail (rangée secondaire) : marge par dépt + turnover
// ───────────────────────────────────────────────────────────
function MargeDeptCard() {
  const rows = MARGE_PAR_DEPT.rows;
  const max = Math.max(...rows.flatMap((r) => [r.v, r.t])) + 4;
  return (
    <div style={{
      background: "var(--surface)", border: "1px solid var(--border)",
      borderRadius: 18, padding: "32px 36px",
      display: "flex", flexDirection: "column", gap: 24, boxShadow: "var(--shadow-1)",
    }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{ width: 42, height: 42, borderRadius: 11, background: "var(--warn-soft)", color: "var(--warn)", display: "grid", placeItems: "center" }}>
            <AfIcon name="step-matrice" size={20}/>
          </div>
          <div>
            <div style={{ fontSize: 15, fontFamily: "var(--font-mono)", color: "var(--fg-2)", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: 700 }}>Marge par département</div>
            <div style={{ fontSize: 13, color: "var(--fg-3)", marginTop: 4 }}>5 départements · actuel vs cible</div>
          </div>
        </div>
        {MARGE_PAR_DEPT.baked && (
          <span className="confirm-badge" style={{ background: "color-mix(in srgb, var(--warn) 18%, transparent)", color: "var(--warn)" }}>BAKED · à fiabiliser</span>
        )}
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        {rows.map((r) => {
          const ok = r.v >= r.t;
          const vPct = (r.v / max) * 100;
          const tPct = (r.t / max) * 100;
          return (
            <div key={r.dept} style={{ display: "grid", gridTemplateColumns: "130px 1fr 110px", alignItems: "center", gap: 18 }}>
              <span style={{ fontSize: 15, color: "var(--fg-1)", fontWeight: 500 }}>{r.dept}</span>
              <div style={{ position: "relative", height: 12, borderRadius: 999, background: "var(--bg-elev-1)", overflow: "visible" }}>
                <div style={{
                  position: "absolute", left: 0, top: 0, bottom: 0,
                  width: `${vPct}%`, borderRadius: 999,
                  background: ok ? "var(--ok)" : "var(--warn)",
                }}/>
                <div style={{
                  position: "absolute", left: `${tPct}%`, top: -5, bottom: -5,
                  width: 2, marginLeft: -1, background: "var(--fg-1)", borderRadius: 1, opacity: 0.7,
                }}/>
              </div>
              <div style={{ display: "flex", alignItems: "baseline", gap: 5, justifyContent: "flex-end" }}>
                <span style={{ fontFamily: "var(--font-mono)", fontSize: 24, fontWeight: 700, color: ok ? "var(--ok)" : "var(--warn)", fontVariantNumeric: "tabular-nums", letterSpacing: "-0.02em" }}>{r.v}<span style={{ fontSize: 13, color: "var(--fg-3)" }}>%</span></span>
                <span style={{ fontSize: 12, fontFamily: "var(--font-mono)", color: "var(--fg-3)" }}>/ {r.t}%</span>
              </div>
            </div>
          );
        })}
      ...
      <div style={{ fontSize: 13, color: "var(--fg-3)", lineHeight: 1.6, paddingTop: 18, borderTop: "1px dashed var(--border-soft)" }}>
        Source agrégée 4 reports Belaza + 1 export Facturation · pas de réconciliation auto · pipeline data fiable visé Q3.
      </div>
    </div>
  );
}
...
function TurnoverCard() {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{ width: 42, height: 42, borderRadius: 11, background: "var(--ok-soft)", color: "var(--ok)", display: "grid", placeItems: "center" }}>
            <AfIcon name="users" size={20}/>
          </div>
          <div>
            <div style={{ fontSize: 15, fontFamily: "var(--font-mono)", color: "var(--fg-2)", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: 700 }}>Turnover agrégé</div>
            <div style={{ fontSize: 13, color: "var(--fg-3)", marginTop: 4 }}>12 mois glissants · plus bas = mieux</div>
          </div>
        </div>
        <span className="confirm-badge" style={{ background: "color-mix(in srgb, var(--err) 16%, transparent)", color: "var(--err)" }}>B17 · agrégat · 0 nominatif</span>
      </div>

      <div style={{ display: "flex", alignItems: "baseline", gap: 14, flexWrap: "wrap" }}>
        <span style={{ fontFamily: "var(--font-mono)", fontSize: 92, fontWeight: 600, color: "var(--ok)", letterSpacing: "-0.045em", lineHeight: 0.9, fontVariantNumeric: "tabular-nums" }}>{k.valueDisplay}</span>
        <span style={{ fontFamily: "var(--font-mono)", fontSize: 28, color: "var(--fg-3)" }}>{k.unit}</span>
        <span style={{ display: "inline-flex", alignItems: "baseline", gap: 7, padding: "7px 13px", borderRadius: 8, background: "var(--bg-elev-1)", border: "1px solid var(--border-soft)", whiteSpace: "nowrap" }}>
          <span style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--fg-3)", textTransform: "uppercase", letterSpacing: "0.12em", fontWeight: 700 }}>cible</span>
          <span style={{ fontFamily: "var(--font-mono)", fontSize: 22, fontWeight: 700, color: "var(--fg-1)" }}>{k.targetDisplay}</span>
        </span>
      </div>

      <div style={{ paddingTop: 6 }}>
        <Gauge value={k.value} min={k.gaugeMin} max={k.gaugeMax} target={k.target} invert/>
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 10, fontSize: 11, fontFamily: "var(--font-mono)", color: "var(--fg-3)" }}>
          <span>0%</span><span>15%</span>
        </div>
      </div>

      <div style={{ fontSize: 14, color: "var(--fg-3)", lineHeight: 1.6, marginTop: "auto" }}>{k.caption}</div>
    </div>
  );
}

function RentDetailBlock() {
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 22, flexWrap: "wrap" }}>
        <div style={{ width: 5, height: 28, background: "var(--accent)", borderRadius: 2, opacity: 0.55 }}/>
        <h3 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: "var(--fg-1)", letterSpacing: "-0.015em", textTransform: "uppercase" }}>Rentabilité · détail</h3>
        <span style={{ fontSize: 14, color: "var(--fg-3)" }}>(décomposition par dépt + capital humain)</span>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(420px, 1fr))", gap: 24 }}>
        <MargeDeptCard/>
        <TurnoverCard/>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 18, flexWrap: "wrap" }}>
        <div style={{ width: 5, height: 28, background: "var(--err)", borderRadius: 2 }}/>
        <h3 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: "var(--fg-1)", letterSpacing: "-0.015em", textTransform: "uppercase" }}>DDV à valider · <span style={{ color: "var(--err)" }}>{items.length} à valider</span></h3>
        <span style={{ fontSize: 14, color: "var(--fg-3)" }}>(&gt;5 M MGA ou cross-threshold · goulot CFO)</span>
        <span style={{ marginLeft: "auto", fontSize: 12, color: "var(--fg-3)", fontFamily: "var(--font-mono)" }}>source · ddv-baked actor_current cfo-001 · WRITES_LIVE=false · M13</span>
      </div>
...
function AlertesHajaBlock() {
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 18, flexWrap: "wrap" }}>
        <div style={{ width: 5, height: 28, background: "var(--warn)", borderRadius: 2 }}/>
        <h3 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: "var(--fg-1)", textTransform: "uppercase", letterSpacing: "-0.015em" }}>Alertes Haja · changement de nature</h3>
        <span style={{ fontSize: 14, color: "var(--fg-3)" }}>(pattern structurel · pas le bruit opérationnel)</span>
      </div>
...
function Sidebar({ active, onTab }) {
  return (
    <aside style={{ width: 240, flexShrink: 0, background: "var(--sidebar-bg)", borderRight: "1px solid var(--border)", display: "flex", flexDirection: "column" }}>
      <nav style={{ display: "flex", flexDirection: "column", gap: 6, padding: 16, flex: 1 }}>
        {NAV.map((n) => {
          const a = active === n.id;
          return (
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--fg-1)", display: "flex", flexDirection: "column" }} data-screen-label="Cockpit Ketsiah CFO">
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
        ...
        <main style={{ flex: 1, overflow: "auto", padding: "48px 56px" }}>
          <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", flexDirection: "column", gap: 48 }}>
            {tab === "vue" && (
              <>
                <CockpitHeader/>

                {/* 2-col : main + rail (le cash vit UNIQUEMENT dans le rail) */}
                <div className="vue-grid" style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 340px", gap: 36, alignItems: "start" }}>
                  <div style={{ display: "flex", flexDirection: "column", gap: 56, minWidth: 0 }}>
                    {/* 1. SO·1 trajectoire (PREMIER · owner) */}
                    <SOBand/>

                    {/* 2. HERO RENTABILITÉ & BUDGET */}
                    <HeroRentBlock/>

                    {/* 3. Rentabilité — détail */}
                    <RentDetailBlock/>

                    {/* 4. DDV à valider — compteur dans l'en-tête */}
                    <DdvTable items={ddvList} onAction={(it) => setOpenDdv(it)}/>

                    {/* 5. Synthèse Haja + alertes */}
                    <HajaSynthCard/>
                    <AlertesHajaBlock/>

                    {/* 6. Bas — lecture seule + conformité compact 2-col */}
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(360px, 1fr))", gap: 14 }}>
                      <ConcentrationCard/>
                      <ConformiteCard/>
                    </div>
                  </div>
                  <div className="vue-rail" style={{ display: "flex", flexDirection: "column", gap: 18, position: "sticky", top: 0 }}>
                    <CashNetCard/>
                    <WhatIfCard/>
                  </div>
                </div>
      ...
      <style>{`
        ...
        @media (max-width: 1080px) {
          .vue-grid { grid-template-columns: 1fr !important; }
          .vue-rail { position: static !important; }
        }
      `}</style>
    </div>
  );
